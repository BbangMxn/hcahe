// Package buffer는 비동기 쓰기를 위한 Write Buffer를 구현합니다.
package buffer
...
//
// 특징:
// - 락 없는 채널 기반 설계로 높은 동시성
// - 같은 키에 대한 중복 쓰기 병합 (coalescing)
// - 배치 플러시로 네트워크 라운드트립 최소화
// =============================================================================
...
// New는 새로운 WriteBuffer를 생성합니다.
//
// Parameters:
//   - size: 버퍼 최대 크기
//   - flushInterval: 자동 플러시 주기
//   - flushFunc: 플러시 시 호출되는 함수
func New(size int, flushInterval time.Duration, flushFunc FlushFunc) *WriteBuffer {
	if size <= 0 {
		size = 1000
	}
	if flushInterval <= 0 {
...
func (wb *WriteBuffer) Add(entry *core.Entry, targetLayers []int) {
	if wb.closed {
		return
	}

	be := &BufferEntry{
		Entry:        entry.Clone(), // 복사본 저장
